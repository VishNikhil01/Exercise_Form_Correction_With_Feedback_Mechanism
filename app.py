...
# # Improved webcam initialization logic
# webcam_options = [0, 1]  # Default webcam indices for iRuin and iVCam webcams
# camera = None
# for index in webcam_options:
#     print(f"Trying webcam index: {index}")
#     temp_camera = cv2.VideoCapture(index)
#     if temp_camera.isOpened():
#         print(f"Webcam index {index} is accessible.")
#         camera = temp_camera
#         break
#     else:
#         print(f"Webcam index {index} is not accessible.")

# if camera is None:
#     st.error("Unable to access any webcam. Please ensure a webcam is connected and not in use by another application.")
#     running = False
# else:
#     # Reduce video resolution to 640x480 for better performance
#     camera.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
...
#         # Enhanced debugging for YOLOv5 detection
#         print("Starting YOLOv5 detection...")
#         resized_frame = cv2.resize(frame, (640, 640))
#         results_yolo = model(resized_frame)
#         print(f"Raw detection results: {results_yolo.pred[0]}")

#         if results_yolo.pred[0] is not None and len(results_yolo.pred[0]) > 0:
#             for det in results_yolo.pred[0]:
#                 c1, c2 = det[:2].int(), det[2:4].int()
#                 cls, conf, *_ = det
#                 label = f"person {conf:.2f}"

#                 if conf >= 0.5:  # Adjusted threshold
#                     print(f"Detected person with confidence {conf:.2f} at {c1}, {c2}")
#                     # Convert tensors to tuples for OpenCV functions
#                     cv2.rectangle(frame, tuple(c1.tolist()), tuple(c2.tolist()), (0, 255, 0), 2)
#                     cv2.putText(frame, label, (c1[0].item(), c1[1].item() - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
#         else:
#             print("No detections found.")

#         # Object detection using YOLOv5
#         # Resize the input frame to match YOLOv5's expected input size
#         resized_frame = cv2.resize(frame, (640, 640))
#         results_yolo = detect_objects(resized_frame)

#         # Debug detection results
#         print(f"Detection results: {results_yolo}")
...
#                     if conf >= 0.5:  # Adjusted threshold from 0.7 to 0.5 for better detection
#                         # Convert c1 and c2 to tuples
#                         c1 = (c1[0].item(), c1[1].item())
#                         c2 = (c2[0].item(), c2[1].item())
